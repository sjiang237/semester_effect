...
```{r}
ggplot(data = d_try) + 
  geom_bar(aes(x = days_since_semester_start), fill = "blue") + # Adjust bar colors
...
ggplot(data = d_try) + 
  geom_bar(aes(x= days_since_semester_start), fill="pink") +
  theme_minimal() +  # Use a clean theme
  labs(
...
ggplot(data = d_matrices) + 
  geom_bar(aes(x= days_since_semester_start), fill="yellow")+
  theme_minimal() +  # Use a clean theme
  labs(
    title = "Enrollment Distribution of Participants",
    x = "Days Since Semester Start",
...
```{r}
ggsave(plot = distribution_4, 
       filename = file.path(figure_path, 'distribution_4.png'), 
